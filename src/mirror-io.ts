/**
 * LOCAL_MIRROR_PATH support for graph-view interop.
 *
 * When LOCAL_MIRROR_PATH is set, remote-memory persists every mutation to a
 * local JSONL file using the anthropic memory MCP's canonical line format
 * (with optional createdAt/updatedAt metadata extensions). graph-view reads
 * and writes that same file directly, so the user gets a UI on top of a
...
function serializeJsonl(graph: { entities: Map<string, Entity>; relations: Relation[] }): string {
  const lines: string[] = [];
  // anthropic's convention: entities first, relations second. Matches both
  // their own save format and graph-view's renormalized output.
...
/**
 * Synchronizes a MemoryGraphManager with a local JSONL mirror file.
 *
 * Lifecycle:
 *   1. constructor only stashes the path
 *   2. init() — call once during server bootstrap. Reads existing mirror if
 *      present and replaces in-memory graph with its contents. If the file
 *      does not exist, the caller should populate in-memory from elsewhere
 *      (e.g. GitHub pull) and then call writeMirror() to seed the file.
 *   3. before each tool dispatch: maybeReload()
 *   4. after each mutation: writeMirror()
 */
export interface SyncBaseline {
  /** GitHub blob SHA observed at last successful pull/push. Empty string = file did not exist remotely. */
  remoteSha: string;
...
export class MirrorManager {
   ...
   * GitHub SHA + graph digest at the last successful pull/push, so that
   * divergence (both sides changed) can be detected across process restarts.
   *
   * Sidecar lives next to the mirror file (mirror.jsonl.sync-state.json) and
   * is never read or written by graph-view — invariant maintained by the
   * mirror format contract.
   */
  async readBaseline(): Promise<SyncBaseline | null> {
    try {
      const text = await fs.readFile(this.sidecarPath, 'utf-8');
      const parsed = JSON.parse(text);
      if (typeof parsed?.remoteSha === 'string' && typeof parsed?.mirrorDigest === 'string') {
        return { remoteSha: parsed.remoteSha, mirrorDigest: parsed.mirrorDigest };
      }
    } catch {
